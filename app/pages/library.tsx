...
type MinimalContent = Pick<ContentItem, 'id' | 'type'>;
...
export function LibraryPage() {
        ...
        const batchPromises = batch.map(async (statusItem) => {
          try {
            ...
            if (contentData && authorData) {
              const maybeWithDuration = contentData as { duration?: string };
              const resolvedDuration = typeof maybeWithDuration.duration === 'string'
                ? maybeWithDuration.duration
                : statusItem.content_type === 'article'
                  ? '5 min read'
                  : '30 min';
  ...
  const handleAddToStatus = async (item: MinimalContent, status: string) => {
    if (!user) return;
