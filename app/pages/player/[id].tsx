...
export function PlayerPage() {
  ...
  useEffect(() => {
    ...
    const handleAudioPause = () => {
      if (listeningStartTime && !hasCountedAsListener) {
        ...
        if (listeningDuration >= 30000) { // 30 seconds
          recordRealListener();
        }
      }
    };
  ...
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        ...
        const [ratingsResult, userRatingResult, userLikeResult, userBookmarkResult] = await Promise.all([
          // Get all ratings
          supabase
            .from('ratings')
            .select('rating')
        ...
        setUserRating(processedContent.user_rating ?? 0);
    ...
    const loadComments = async () => {
      ...
      try {
        ...
        if (commentsData && commentsData.length > 0) {
          ...
          if (user) {
              try {
                const savedLikes = localStorage.getItem(`comment_likes_${user.id}_${contentId}`);
                const savedLikeCounts = localStorage.getItem(`comment_like_counts_${contentId}`);
            ...
            .map(comment => {
                ...
                .map(reply => {
                  ...
                  return {
                    ...
                    user: {
                      ...
                      name: replyProfile?.name || replyProfile?.username || 'Anonymous',
                      username: replyProfile?.username || 'anonymous',
                      avatar_url: replyProfile?.avatar_url || `https://source.unsplash.com/random/100x100?face&sig=${reply.user_id}`
                    },
              ...
              return {
                ...
                user: {
                  ...
                  avatar_url: profile?.avatar_url || `https://source.unsplash.com/random/100x100?face&sig=${comment.user_id}`
                },
    ...
    try {
      ...
      setContent(prev => prev ? {
        ...
        avg_rating: prev.user_rating 
          ? ((prev.avg_rating * prev.total_ratings) - prev.user_rating + rating) / prev.total_ratings
          : ((prev.avg_rating * prev.total_ratings) + rating) / (prev.total_ratings + 1)
      } : null);
    } catch (error) {
      console.error('Error rating content:', error);
      setUserRating(content?.user_rating ?? 0);
    }
  };
    ...
    try {
      ...
      const newCommentObj: Comment = {
        ...
        user: {
          ...
          avatar_url: userProfile?.avatar_url || `https://source.unsplash.com/random/100x100?face&sig=${user.id}`
        },
        likes: 0,
        is_liked: false,
        replies: []
  ...
  const handleCommentLike = async (commentId: string) => {
    ...
    try {
      ...
      try {
        localStorage.setItem(`comment_likes_${user.id}_${contentId}`, JSON.stringify(updatedUserLikes));
        localStorage.setItem(`comment_like_counts_${contentId}`, JSON.stringify(updatedCommentLikes));
      } catch (error) {
    ...
    <div className="min-h-screen bg-background pb-32 pt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className={`text-center lg:text-left px-4 lg:px-0`}>
              <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight text-foreground reader-title ${getTextLanguageClass(content.title)}`}>
                {content.title}
              </h1>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border rounded-xl p-4 sm:p-6 shadow-sm mx-4 lg:mx-0">
              <h3 className="text-base sm:text-lg font-semibold mb-4">Rate this {contentType}</h3>
              
              {/* Overall Rating Display */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 mb-6">
                <div className="flex items-center gap-2">
                  <div className="text-xl sm:text-2xl font-bold">{(content.avg_rating || 0).toFixed(1)}</div>
                ...
                <span className="text-xs sm:text-sm text-muted-foreground">
                  ({(content.total_ratings || 0).toLocaleString()} {(content.total_ratings || 0) === 1 ? 'rating' : 'ratings'})
                </span>
              </div>
              ...
              {user ? (
                <div>
                  <p className="text-xs sm:text-sm font-medium mb-3">Your rating:</p>
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <button
                        key={i}
            ...
            <div className="bg-card border rounded-xl p-4 sm:p-6 shadow-sm mx-4 lg:mx-0">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-6">
                <h3 className="text-base sm:text-lg font-semibold">Discussions</h3>
                <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                  <MessageSquare className="w-4 h-4" />
                 <span>
                   {comments.reduce((total, comment) => total + 1 + (comment.replies?.length || 0), 0)} 
                   {' '}
                   {comments.reduce((total, comment) => total + 1 + (comment.replies?.length || 0), 0) === 1 ? 'comment' : 'comments'}
                 </span>
                </div>
              </div>
              ...
              {user ? (
                ...
                <div className="mb-4 sm:mb-6 p-3 sm:p-4 md:p-6 rounded-lg bg-muted/30 text-center">
                  <p className="text-xs sm:text-sm text-muted-foreground mb-3">Join the discussion</p>
                  <button
                    onClick={() => navigate('/signin')}
              ...
              <div className="space-y-4">
                {comments.length > 0 ? (
                  comments.map((comment) => (
                    <div key={comment.id} className="space-y-3">
                      ...
                      <div className="flex gap-2 sm:gap-3">
                        <button
                          onClick={() => handleProfileClick(comment.user.id, comment.user.username)}
                          ...
                          {replyingTo === comment.id && (
                            <div className="mt-4 flex gap-3">
                              <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full overflow-hidden bg-muted flex-shrink-0">
                                <img
                                  src={user?.user_metadata?.avatar_url || `https://source.unsplash.com/random/100x100?face&sig=${user?.id}`}
                                  alt="Your avatar"
                                  className="w-full h-full object-cover"
                                />
                              </div>
                          ...
                          {comment.replies && comment.replies.length > 0 && (
                            <div className="mt-4 space-y-3">
                              {comment.replies.map((reply) => (
                                <div key={reply.id} className="flex gap-2 ml-4 pl-4 border-l-2 border-muted">
                                  <button
                                    onClick={() => handleProfileClick(reply.user.id, reply.user.username)}
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs sm:text-sm mb-2 leading-relaxed">{reply.content}</p>
                                    <div className="flex items-center gap-4">
                                      <button 
                  ...
                  <div className="text-center py-6 sm:py-8 text-muted-foreground">
                    ...
                    <p className="text-base sm:text-lg font-medium mb-2">No comments yet</p>
                    <p className="text-xs sm:text-sm">Be the first to share your thoughts about this {contentType}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {shareStatus !== 'idle' && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 px-4 py-2 bg-popover border rounded-lg shadow-lg text-sm flex items-center gap-2 z-50">
          ...
          <span>{shareStatus === 'copied' ? 'Link copied to clipboard' : 'Shared successfully'}</span>
        </div>
      )}
    </div>
  );
