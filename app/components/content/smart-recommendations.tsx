...
export function SmartRecommendations({ currentContent }: SmartRecommendationsProps) {
  const { user } = useAuth();
      ...
      try {
          ...
          if (historyData) {
            ...
            historyData.forEach(item => {
              ...
              if (item.content_type === 'audiobook' && item.audiobooks) {
                ...
                itemCategories = ab?.categories || (ab?.category ? [ab.category] : []);
                ...
                itemCategories = pod?.categories || (pod?.category ? [pod.category] : []);
              }
        ...
        const processContent = (data: any[], type: ContentItem['type']) => {
          data?.forEach((item) => {
            ...
            const categoryMatchScore = categoryFilter.reduce((score, cat) =>
              itemCategories.includes(cat) ? score + 1 : score, 0
            );
